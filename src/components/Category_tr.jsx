...
function Category_tr({ category }) {
  return (
    <tr className="table-row">
      ...
      <td>
        {" "}
        {format(new Date(category.createdAt), "dd MMM yy", { locale: enUS })}
      </td>
      <td>
        {" "}
        {format(new Date(category.updatedAt), "dd MMM yy", { locale: enUS })}
      </td>
      <td>
        <NavLink to={`/categories/edit/${category.slug}`}>
          <i className="bi bi-pencil-fill text-success pencil"></i>
        </NavLink>
      </td>
    </tr>
  );
